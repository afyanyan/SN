...
class SN:
    ...
    def read_data(self):
        ...
        self.t_last_nondetection = df_bkg['MJD'].iloc[-1]
        self.t_first_detection = df_lc['MJD'].iloc[0]

        return self.time, self.mag, self.mag_err
...
def gaussian_model(time, mag, mag_err, t_i, t_f):
    #Prior Data
    t_sbo = pyro.sample("t_sbo", dist.Uniform(t_i, t_f))
    amplitude = pyro.sample("amplitude", dist.HalfNormal(torch.tensor(2.0)))
    sigma = pyro.sample("sigma", dist.HalfNormal(torch.tensor(3.0)))

    #Gaussian Curve
    dt = time - t_sbo
    predicted_mag = mag.max() - amplitude * torch.exp(-0.5 * (dt / sigma) ** 2)

    #Likelihood
    with pyro.plate("obs", len(time)):
        pyro.sample("mag_obs", dist.Normal(predicted_mag, mag_err), obs=mag)
